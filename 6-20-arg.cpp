...
map<string,vector<string> > func_arguments; // аргументы функций
map<string,double> table_stack; // временное хранилище значений уже определенных 
								// переменных для запуска функций
...
double prim(bool get)
{
  ...
  case NAME:
  {
	if (is_function[string_value])
	{
		...
		vector<double> call_args;
		char tok = get_token();
		if (tok == LP)
		{
			string arg_name = "";
			while (cin.get(tok) && tok != RP)
			{
				if (tok == ',')
				{
		...
		// now putting function body into input stream
	...
	else
	{
		...
		vector<string>& arguments = func_arguments[string_value];
		if (tok == LP)
		{
			string arg_name = "";
			while (cin.get(tok) && tok != RP)
			...
			tok=get_token();
			/*
			vector<string>::iterator the_iterator = arguments.begin();
			while( the_iterator != arguments.end() ) 
			{
				cout << "argument: " << *the_iterator << '\n';
				the_iterator++;
			}
			 */
		}
